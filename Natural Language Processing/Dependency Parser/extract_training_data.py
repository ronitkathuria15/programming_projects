...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        result = [self.word_vocab['<NULL>']] * 6
        for i in range(3):
            if i < len(state.stack):
                id = state.stack[-(i+1)]
                if id == 0:
                    result[i] = self.word_vocab['<ROOT>']
                else:
                    word = words[id]
                    if word not in self.word_vocab:
                        result[i] = self.word_vocab['<UNK>']
                    elif pos[id] == 'NNP':
                        result[i] = self.word_vocab['<NNP>']
                    elif pos[id] == 'CD':
                        result[i] = self.word_vocab['<CD>']
                    else:
                        result[i] = self.word_vocab[word]
            if i < len(state.buffer):
                id = state.buffer[-(i+1)]
                if id == 0:
                    result[i+3] = self.word_vocab['<ROOT>']
                else:
                    word = words[id]
                    if word not in self.word_vocab:
                        result[i+3] = self.word_vocab['<UNK>']
                    elif pos[id] == 'NNP':
                        result[i+3] = self.word_vocab['<NNP>']
                    elif pos[id] == 'CD':
                        result[i+3] = self.word_vocab['<CD>']
                    else:
                        result[i+3] = self.word_vocab[word]
        return np.array(result)

    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        output_id = self.output_labels[output_pair]
        return tf.keras.utils.to_categorical(output_id, num_classes=91)
